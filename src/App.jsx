...
import bsbUniverseImg from './assets/bsb-universe.jpg'
import battlePassNewImg from './assets/battle-pass-new.jpg'
import bsbLogoImg from './assets/bsb-logo.jpg'
...
function App() {
  ...
  const services = [
    ...
    {
      id: 'trading-advantages',
      title: 'Avantages',
      icon: Bot,
      image: tradingBotImg,
      description: "Simplicité et Accessibilité : Le robot de trading fonctionne de manière autonome, sans nécessiter de connaissances approfondies en trading.\nDisponibilité 24/7 : Le robot travaille en continu, générant potentiellement des revenus à tout moment, même pendant que vous dormez ou profitez de votre journée.\nContrôle Total : Les fonds restent sur votre propre compte chez Vantage FX, et vous êtes le seul à pouvoir retirer ou déposer de l’argent.\nAlignement des Intérêts : La rémunération de BSB se fait uniquement sur 25% des bénéfices réalisés, ce qui motive à maximiser les performances et à éviter les pertes."
    },
    {
      id: 'trading-disadvantages',
      title: 'Inconvénients',
      icon: Bot,
      image: tradingBotImg,
      description: "Risque d’Investissement : Comme tout investissement, il existe un risque de perte, même si le robot est conçu pour maximiser les gains."    },
    {
      id: 'battlepass',
      title: 'BATTLE PASS',
      icon: Trophy,
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800">
      ...
      <header className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center text-white w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-8 sm:mb-12 px-4">
            <Button 
            ...
            {services.map((service, index) => {
              ...
              return (
                <div
                  key={service.id}
                  id={service.id}
                  className={`group relative bg-gradient-to-br from-slate-800/60 to-slate-900/80 backdrop-blur-sm rounded-2xl overflow-hidden border transition-all duration-500 transform hover:scale-105 hover:shadow-2xl ${
                    service.id === 'trading' ? 'border-cyan-500/30 hover:border-cyan-400/60 hover:shadow-cyan-500/20' :
                ...
                >
                  <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${
                    service.id === 'trading' ? 'bg-gradient-to-br from-cyan-500/10 to-blue-500/10' :
                  ...
                  <div className="relative h-48 sm:h-64 overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4">
                      <div className={`backdrop-blur-sm rounded-full p-2 sm:p-3 border ${
                        service.id === 'trading' ? 'bg-cyan-500/20 border-cyan-400/30' :
                        service.id === 'battlepass' ? 'bg-purple-500/20 border-purple-400/30' :
                        'bg-amber-500/20 border-amber-400/30'
                      }`}>
                        <Icon className={`h-4 w-4 sm:h-6 w-6 ${
                          service.id === 'trading' ? 'text-cyan-400' :
                  ...
                  <div className="p-4 sm:p-6">
                    <h3 className={`text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4 transition-colors duration-300 ${
                      service.id === 'trading' ? 'group-hover:text-cyan-400' :
                    ...
                    <Button 
                      variant="ghost" 
                      className={`transition-all duration-300 group-hover:translate-x-2 text-xs sm:text-sm ${
                        service.id === 'trading' ? 'text-cyan-400 hover:text-white hover:bg-cyan-500/20' :
            ...
            <div className="bg-gradient-to-br from-slate-800/60 to-slate-900/80 backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-blue-500/20 hover:border-blue-400/40 transition-all duration-300 group">
              <div className="flex flex-col items-center text-center">
                ...
                <Button 
                  ...
                  onClick={() => window.open('mailto:contact@bsbridge.com', '_blank')}
                >
                  Envoyer un Email
                </Button>
              </div>
